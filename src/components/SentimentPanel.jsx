...
// Sentiment is generated by Groq (primary), Cerebras (second fallback),
// then Gemini (last resort) — see etl/sentiment_gemini.py. Label shown
// next to the AI-generated content should reflect whichever provider
// actually produced it, not be hardcoded.
...
export default function SentimentPanel({ pipelineData }) {
  ...
  const d = sentiment
  const isUnavailable = d.source === 'fallback' || d.model === 'fallback-static'
  const providerLabel = PROVIDER_LABELS[d.source] || 'AI'
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 py-16">
      <div className="text-center mb-10">
        ...
        <h2 className="text-3xl font-light text-gray-800">AI policy sentiment analysis</h2>
        <p className="text-sm text-gray-500 mt-3 max-w-lg mx-auto">
          Daily analysis generated by Groq, with Cerebras and Gemini as fallbacks, from consolidated mobility, fuel, ridership, and WFH data
        </p>
      </div>

      {isUnavailable ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          className="glass rounded-3xl p-8 mb-6 border border-amber-100">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl bg-amber-50 flex items-center justify-center flex-shrink-0">
              <AlertTriangle size={20} color="#BA7517" />
            </div>
            <div>
              <div className="text-sm font-semibold text-gray-700">AI sentiment analysis temporarily unavailable</div>
              <p className="text-sm text-gray-600 leading-relaxed mt-2">
                Groq, Cerebras, and Gemini API quotas were all exhausted on the last pipeline run — no
                AI-generated signals for today. The MF-Index itself (<strong>{d.overall_score}/100</strong>) is
                still computed directly from live data, unaffected by this.
              </p>
              <p className="text-xs text-gray-400 mt-3">{d.recommendation}</p>
            </div>
          </div>
        </motion.div>
      ) : (
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center gap-2 text-xs text-gray-400">
          <span className="px-2 py-1 rounded-full bg-blue-50 text-blue-600 font-medium">{providerLabel} · {d.model}</span>
      ...
      <div className="glass rounded-3xl p-6 border border-blue-100">
        <div className="text-xs font-medium text-blue-600 uppercase tracking-widest mb-3">{providerLabel} recommendation</div>
